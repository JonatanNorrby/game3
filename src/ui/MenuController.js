...
export class MenuController {
  ...
  renderBosses() {
    ...
    this.bossRoster.forEach((entry, index) => {
      ...
      card.innerHTML = `
        <span class="boss-card-title">${entry.name}</span>
        <span class="boss-card-description">${entry.description}</span>
        <span class="boss-card-state">${entry.unlocked ? 'Fight boss' : `Locked${entry.unlockRequirement ? ` · ${entry.unlockRequirement}` : ''}`}</span>
      `;
